...
class ImageProcessor:
    ...
    def apply_gemini_watermark(self, image: Image.Image, watermark: Image.Image, opacity: float = 0.25) -> Optional[Image.Image]:
        is_safe, _ = self.check_image_safety(image)
        if not is_safe:
            return None
        ...
        alpha = resized_watermark.getchannel("A")
        new_alpha = alpha.point(lambda v: int(255 * opacity) if v > 10 else 0)
        resized_watermark.putalpha(new_alpha)
    ...
    def _calculate_doubao_position(self, img_width: int, img_height: int, wm_width: int, wm_height: int) -> Tuple[int, int]:
        ...
        margin_bottom = int(img_height * 0.03)
